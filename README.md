...
- **Порты и связи.** Кабель документируется как связь двух портов. Статус
  ...
  связи, поэтому рассинхрона не бывает. Список кабелей отбирается по
  железке на любом из концов — по коду или по названию, — и по подвешенному
  концу; на каждом конце видно и код, и название.
- **Схема связей.** Автоматическая раскладка по кабелям (ELK): ядро сети
  ...
  как рамка со своим содержимым, подгруппа — внутри неё. Схема, которую ещё
  не расставляли руками, раскладывается сразу при открытии — двести железок
  в пяти цехах читаются с первого взгляда, а не после нажатия «Разложить».
  Чем дальше отодвинули, тем меньше на схеме написано: сначала уходят номера
  портов у концов кабелей, потом строки под названием, а на общем виде
  карточки читаются цветными плашками по модели, и схема превращается в
  карту цехов и шкафов с укрупнёнными подписями. Перетаскивание
