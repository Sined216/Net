...
def test_csv_is_parsed_with_unknown_columns_kept(client, headers):
    ...
    # Столбцы, которым нет места в модели, сохранены как есть. «Расположение»
    # среди них: у устройства такого поля больше нет (его заменила группа на
    # топологии), но из файла оно не пропадает — человек видит его при
    # переносе строки и решает сам, в какую группу её положить.
    assert first["extra"] == {"Расположение": "Цех 1", "Инвентарный номер": "ИНВ-001"}
