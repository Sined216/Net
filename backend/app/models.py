...
class TopologyGroup(Base):
    """Группа устройств — отдельный от тегов параметр: ровно одна группа
    на устройство (или ни одной). Теги множественные и вложенность вида
    «цех — участок — линия» с ними не выразить однозначно (неясно, какая
    из двух вложенных меток устройства — «самая внутренняя») — это
    специально узкое поле под одну строгую иерархию.

    Раньше группа была ещё и рамкой-кластером на схеме связей: своя область
    на плане со своими координатами (см. историю колонок `x`/`y`/`width`/
    `height`, снятых как мёртвые после отказа от рамок). Группировка как
    данные никуда не делась — заводится и правится на своей странице
    («Группы»), схема её просто больше не рисует.

    Группы вкладываются друг в друга (parent_id): цех — участок — линия.
    Устройство при этом принадлежит ровно одной, самой внутренней —
    остальные охватывают его через вложенность, а не через вторую запись."""
    __tablename__ = "topology_groups"
    id = Column(Integer, primary_key=True)
    site_id = Column(Integer, ForeignKey("sites.id", ondelete="CASCADE"), nullable=False, index=True)
    name = Column(Text, nullable=False)
    color = Column(Text)
    # Вид группы: обычная (цех, участок, линия) или шкаф — реальная
    # физическая железка, а не организационная область. У шкафа не бывает
    # подгрупп: это конец дерева, что и держит проверка на роутере при
    # смене вида и при вложении.
