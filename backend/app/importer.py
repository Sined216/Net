...
HEADERS = {
    ...
    "mac": ["mac", "macадрес", "macaddress", "мак", "макадрес"],
    # «Расположение» здесь больше нет: у устройства такого поля не осталось,
    # его заменила группа на топологии. Колонка файла при этом не пропадает —
    # неопознанные заголовки складываются в `extra`, и человек видит их при
    # переносе строки.
