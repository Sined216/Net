...
# Отдельный, куда более щедрый предел времени — для «Обойти всё дерево
# MIB»: это осознанно медленное действие по отдельной кнопке, не часть
# обычного опроса. Число собираемых OID сознательно не ограничено — с
# запросом на «увидеть всё» плохо сочетается тихо обрезанный список;
# единственная граница — общее время (см. ниже, обрыв по времени всё
...
async def _run_raw_walk(
    ...
    try:
        ...
        # «обойти всё». Ограничивает его только общее время (см.
        # _RAW_WALK_BUDGET_S в raw_walk выше), не число OID.
        trace.append(TraceStep(
