    ...
    for (const table of tables) {
      ...
      table.columns.forEach((column, index) => {
        ...
        graph.addCell(new shapes.standard.Link({
          ...
          // Прямая линия, как и на схеме связей: обходить карточки стороной
          // ей незачем, а прямой отрезок читается короче и однозначнее
          // ломаной. Идёт под карточками (z ниже, чем у таблиц), поэтому не
          // перекрывает колонки, через которые проходит напрямик.
          connector: { name: 'rounded', args: { radius: 8 } },
          // Ниже z самой первой добавленной таблицы (её JointJS проставляет
          // автоматически с единицы): иначе линия ложилась бы поверх неё
          // одной, а над остальными таблицами — правильно, под ними.
          z: 0,
          attrs: {
            line: {
